...
perl /path/to/Mutect_IMAPR/scripts/detect_variants_sentieon.pl -ID $sample_name -mode $input_format -T $tumor_input -N $normal_input -R $fasta_ref -O $out_prefix -thread $thread -ram $ram -gatk $gatk -picard $picard -hisat2 $hisat2 -samtools $samtools -gtf $gtf_ref -gene $genelist_ref -dbsnp $dbsnp_ref -hisat2_reference $hisat_ref -germline $germline_ref -pon $PON_ref
...
#perl /services/tools/imapr/1.0/filter_variants.pl -ID $sample_name -O $out_prefix -R $fasta_ref -igg $igg_ref -hla $hla_ref -pseudo $pseudo_ref -tcga $tcga_PON_ref -radar $radar_ref -darned $darned_ref -redi $REDI_ref -samtools $samtools -bcftools $bcftools

#echo "perl machine_learning.pl -ID $sample_name -O $out_prefix -gtf $gtf_ref"
#perl /services/tools/imapr/1.0/machine_learning.pl -ID $sample_name -O $out_prefix -gtf $gtf_ref
