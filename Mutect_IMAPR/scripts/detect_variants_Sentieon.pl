...
if($modeInput eq 'RNA/DNA'){
	...
	# Changed for rereunning last part of analysis again
	...
	# function changed due to reference file and output format are changed
	variantsCalling2($tumorInputMutectReAlign,$normalInputMutect,$variationIDInput,$outDir,$fastaReference);
	###system("rm -r $outDir/*bedSplit*");
}
# I have deleted the option with RNA/RNA since we will not be using this
else{
...
sub RNAPreProcessParallel{
	...
	#
	#system("cp $outDir/tmp/split.bam $outDir/reAligned_$format.bam");
